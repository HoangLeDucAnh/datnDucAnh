...
if (!empty($_POST)) {
  ...
  $sql = "UPDATE livingroom_dht11_fan_update SET temperature = ?, humidity = ?, status_read_sensor_dht11 = ?, time = ?, date = ? WHERE id = ?";
  $q = $pdo->prepare($sql);
  ...
  while ($found_empty == false) {
    ...
    $sql = 'SELECT * FROM livingroom_dht11_fan_record WHERE id="' . $id_key . '"';
    $q = $pdo->prepare($sql);
    $q->execute();
